...
def generate_final_labels(library_mirror, specific_release_id=None):
    ...
    try:
        ...
        if success:
            ...
            logger.warning(
                "⚠️  Label generation failed, but main process completed successfully"
            )

    except Exception as e:
        logger.error(f"Error generating labels: {e}")
        logger.info(
            "Main process completed successfully despite label generation error"
        )
...
def main():
    ...
    if args.release_id:
        ...
        if args.regenerate_labels and args.regenerate_waveforms:
            if limit:
                logger.separator(
                    f"Regenerating LaTeX labels and waveforms (Limited to {limit} releases)"
                )
            else:
            ...
            if limit:
                logger.separator(
                    f"Regenerating LaTeX labels (Limited to {limit} releases)"
                )
            else:
            ...
            if limit:
                logger.separator(
                    f"Regenerating waveforms (Limited to {limit} releases)"
                )
            else:
        ...
        if args.max:
            limit = args.max
            logger.separator(
                f"Discogs-Only Mode - Syncing {limit} releases (metadata/covers only)"
            )
        elif args.dev:
            limit = 10
            logger.separator(
                "Discogs-Only Mode - Development (10 releases, metadata/covers only)"
            )
        else:
            logger.separator(
        ...
        if args.max:
            limit = args.max
            logger.separator(
                f"Download-Only Mode - Syncing {limit} releases (no analysis)"
            )
        elif args.dev:
            limit = 10
            logger.separator(
                "Download-Only Mode - Development (10 releases, no analysis)"
            )
        else:
        ...
        if args.max:
            limit = args.max
            logger.separator(
                f"Starting Discogs Library Sync (Limited to {limit} releases)"
            )
        elif args.dev:
            limit = 10
            logger.separator(
                "Starting Discogs Library Sync (Development Mode - 10 releases)"
            )
        else:
