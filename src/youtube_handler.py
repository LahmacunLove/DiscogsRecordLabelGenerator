...
def _analyze_track_standalone(task):
    ...
    try:
        ...
        if analysis_needed:
            logger.process(
                f"[{release_info}] Running Essentia analysis for track {track_position}"
            )
            ...
            logger.success(
                f"[{release_info}] Essentia analysis completed for track {track_position}"
            )
        ...
        if waveform_needed:
            try:
                logger.process(
                    f"[{release_info}] Generating waveform for track {track_position}"
                )
                ...
                logger.success(
                    f"[{release_info}] Waveform generation completed for track {track_position}"
                )
            except Exception as e:
                logger.warning(
                    f"[{release_info}] Waveform generation failed for track {track_position}: {e}"
                )

        duration = time.time() - start_time
        logger.success(
            f"[{release_info}] Track {track_position} analysis completed in {duration:.2f}s"
        )
        return {
            "track": track_position,
            "success": True,
...
class YouTubeMatcher:
    ...
    def fetch_single_metadata(self, url):
        ...
        with YoutubeDL(self.ytdl_opts) as ydl:
            try:
                ...
                return {"url": url, "title": None, "author": None, "length": None}
    ...
    def fetch_release_YTmetadata(self, video_urls):
        ...
        if len(video_urls) <= 1:
            ...
            logger.process(
                f"Fetching metadata for {len(video_urls)} YouTube videos in parallel..."
            )

            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                # Start all metadata fetches in parallel
                future_to_url = {
                    executor.submit(self.fetch_single_metadata, url): url
                    for url in video_urls
                }
    ...
    def match_discogs_release_youtube(self, discogs_metadata):
        ...
        if (
                ...
                for j, video in enumerate(videos):
                    ...
                    if track_duration and video_duration:
                        ...
                        if duration_ratio > 2.0 or duration_ratio < 0.5:
                            ...
                            logger.debug(
                                f"Rejecting video '{video_title}' for track '{track_title}': "
                                f"duration ratio {duration_ratio:.2f} (video: {video_duration}s, track: {track_duration}s)"
                            )
                            score = float(
                                "inf"
                            )  # Use infinity to completely exclude from Hungarian algorithm
                        else:
                            # Normal scoring with duration consideration
                            title_score = fuzz.WRatio(
                                track_title.lower(), video_title.lower()
                            )
                            title_artist_score = fuzz.WRatio(
                                f"{track_title.lower()} - {track_artist.lower()}",
                                video_title.lower(),
                            )
                            diff = abs(track_duration - video_duration)
                            duration_score = max(
                                0, 100 - (diff / max(track_duration, 1)) * 100
                            )  # Scale to 0-100
                    ...
                    )  # Keep infinity for excluded matches
            ...
            try:
                ...
                for i, (track_title, track_duration, track_position) in enumerate(
                    track_info
                ):
                    self.matches.append(
                        {
            ...
            if assignment_successful:
                ...
                for i, j in zip(track_indices, video_indices):
                    ...
                    if score_matrix[i][j] == float("inf"):
                        logger.warning(
                            f"No valid YouTube match found for track '{track_title}' (all videos rejected by duration filter)"
                        )
                        self.matches.append(
                            {
                                "discogs_track": track_title,
                        ...
                        final_score = score_matrix[i][
                            j
                        ]  # Score is already positive now
    ...
    def update_metadata_with_youtube_durations(self):
        ...
        for track in metadata.get("tracklist", []):
            ...
            if track_pos and (not current_duration or current_duration.strip() == ""):
                if track_pos in youtube_durations:
                    ...
                    updated_count += 1
                    logger.info(
                        f"Updated track {track_pos} duration: {youtube_durations[track_pos]} (from YouTube)"
                    )
        ...
        if updated_count > 0:
            try:
                ...
                logger.success(
                    f"Updated {updated_count} track durations in metadata.json"
                )
            except Exception as e:
    ...
    def audio_download_analyze(self, release_metadata):
        ...
        for i, match in enumerate(self.matches):
            ...
            track_filename_base = os.path.join(
                self.release_folder, sanitize_filename(track_position)
            )
            ...
            if existing_file:
                logger.info(
                    f"Audio file already exists for track {track_position}: {os.path.basename(existing_file)}"
                )
            ...
            try:
                logger.process(
                    f"Downloading audio for track {track_position}: {match['discogs_track']}"
                )
        ...
        if downloaded_tracks:
            ...
            logger.info(
                f"[{release_folder_name}] Download phase complete. Starting parallel analysis for {len(downloaded_tracks)} tracks..."
            )
            self._parallel_audio_analysis(downloaded_tracks)

        release_folder_name = os.path.basename(self.release_folder)
        logger.success(
            f"[{release_folder_name}] Audio download and analysis completed for {len(downloaded_tracks)} tracks"
        )
        return
    ...
    def audio_download_only(self, release_metadata):
        ...
        for i, match in enumerate(self.matches):
            ...
            if existing_file:
                logger.info(
                    f"[{release_folder_name}] Audio file already exists for track {track_position}: {os.path.basename(existing_file)}"
                )
                downloaded_tracks.append((track_filename_base, track_position))
                continue
            ...
            try:
                logger.process(
                    f"[{release_folder_name}] Downloading audio for track {track_position}: {match['discogs_track']}"
                )
                ...
                if downloaded_file:
                    logger.success(
                        f"[{release_folder_name}] Downloaded: {os.path.basename(downloaded_file)}"
                    )
                    downloaded_tracks.append((track_filename_base, track_position))
                else:
                    logger.error(
        ...
        logger.success(
            f"[{release_folder_name}] Audio download completed for {len(downloaded_tracks)} tracks (no analysis)"
        )
        return
    ...
    def _parallel_audio_analysis(self, downloaded_tracks):
        ...
        optimal_workers, effective_cores, logical_cores, ht_detected = (
            get_optimal_workers(min_workers=1, max_percentage=0.8)
        )
        max_workers = min(len(downloaded_tracks), optimal_workers)

        if ht_detected:
            logger.info(
                f"🔄 [{release_folder_name}] Hyperthreading detected: {logical_cores} logical → {effective_cores} physical cores"
            )
        logger.info(
            f"🔄 [{release_folder_name}] Starting parallel analysis with {max_workers}/{effective_cores} processes"
        )
                ...
                try:
                    ...
                    if result["success"]:
                        if result["error"] != "Already analyzed":
                            logger.success(
                                f"✅ [{release_folder_name}] Track {result['track']} analyzed in {result['duration']:.2f}s"
                            )
                    else:
                        logger.error(
                            f"❌ [{release_folder_name}] Track {result['track']} failed: {result['error']}"
                        )

                except Exception as e:
                    logger.error(
                        f"❌ [{release_folder_name}] Track {track_position} crashed: {e}"
                    )
                    results.append(
                        {
                            "track": track_position,
        ...
        avg_time = sum(r["duration"] for r in results if r["success"]) / max(
            successful, 1
        )
        ...
        logger.info(
            f"   🚀 Parallel speedup: ~{len(downloaded_tracks) * avg_time / max(total_time, 0.1):.1f}x faster than sequential"
        )
    ...
    def download_and_stream_audio(self, youtube_url, output_path="output.ogg"):
        ...
        ffmpeg_cmd = [
            ...
            output_path,
            "-f",
            "f32le",  # PCM: 32-bit float little endian
            "-acodec",
            "pcm_f32le",
            "-ac",
            "1",  # Mono
            "-ar",
            "22050",  # Sample rate for librosa (smaller = faster)
    ...
    def analyze_downloaded_track(self, track_filename_base, track_position):
        ...
        try:
            ...
            if analysis_exists and waveform_exists:
                logger.info(
                    f"Audio analysis and waveform already exist for track {track_position}"
                )
                return True
            ...
            if os.path.getsize(audio_file) == 0:
                logger.error(
                    f"Audio file is empty for track {track_position}: {audio_file}"
                )
                return False
            ...
            if not analysis_exists:
                try:
                    logger.analyze(
                        f"Running Essentia analysis: {os.path.basename(audio_file)}"
                    )
                    analyzer.readAudioFile(ffmpegUsage=False)
                    analyzer.analyzeMusicExtractor()
                    logger.success(
                        f"Essentia analysis completed for track {track_position}"
                    )
                except Exception as e:
                    logger.error(
                        f"Essentia analysis failed for track {track_position}: {e}"
                    )
                    success = False
            ...
            if not waveform_exists:
                try:
                    ...
                    if waveform_success:
                        logger.success(
                            f"Waveform generation completed for track {track_position}"
                        )
                    else:
                        logger.warning(
                            f"Waveform generation failed for track {track_position}"
                        )
                        success = False
                except Exception as e:
                    logger.error(
                        f"Waveform generation failed for track {track_position}: {e}"
                    )
                    success = False
