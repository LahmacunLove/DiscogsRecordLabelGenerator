...
class DiscogsLibraryMirror:
    ...
    def save_cover_art(self, release_id, metadata):
        ...
        logger.debug(f"Cover URL: {imageURL}")
        logger.debug(f"Cover path: {os.path.join(self.release_folder, 'cover.jpg')}")
        
        if not os.path.isfile(os.path.join(self.release_folder, "cover.jpg")):
            if imageURL !=  None:
                try:
                    logger.download(f"Cover art for release {release_id}")
